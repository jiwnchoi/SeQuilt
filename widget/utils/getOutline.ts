import type { IPoint, IRect } from "@/model";

declare global {
  interface Array<T> {
    findRight(predicate: (value: T, index: number, obj: T[]) => unknown): T | undefined;
  }
}
...
Array.prototype.findRight = function <T>(
  ...
  return this.reduceRight((acc: T | undefined, current: T, index: number, array: T[]) => {
    if (acc !== undefined) {
      return acc;
    }
...
function _getNextPoint(currentPoint: IPoint, points: IPoint[]) {
  if (currentPoint.type === "top-left") {
    return points.find(point => point.y === currentPoint.y && point.type === "top-right");
  }
  if (currentPoint.type === "top-right") {
    return points.find(point => point.x === currentPoint.x);
  }
  if (currentPoint.type === "bottom-right") {
    return points.findRight(point => point.y === currentPoint.y && point.type === "bottom-left");
  }
  if (currentPoint.type === "bottom-left") {
...
function getOutline(rects: IRect[]): string {
  ...
  while (points.length > 0) {
    ...
    points = points.filter(point => !(point.x === nextPoint.x && point.y === nextPoint.y));
